...
export function createAcpReadOperations(deps: AcpReadOperationsDeps): ReadOperations {
	...
	const assertNotDirectory = async (absolutePath: string): Promise<void> => {
		try {
			const st = await stat(absolutePath);
			if (st.isDirectory()) {
				throw new Error(`EISDIR: ${absolutePath} is a directory`);
			}
		} catch (err) {
	...
	return {
		async readFile(absolutePath: string): Promise<Buffer> {
			...
			await assertNotDirectory(absolutePath);
